...
import uk.gov.gds.ier.model.{DOB, DateOfBirth, InprogressCrown}
...
class DateOfBirthFormTests 
  ...
  it should "fill and validate correctly" in {
    val filledForm = dateOfBirthForm.fillAndValidate(InprogressCrown(dob = Some(DateOfBirth(Some(DOB(1988, 1, 1)), None))))
    filledForm.errors should be(Nil)
  }

  it should "fill and validate error out correctly" in {
    val filledForm = dateOfBirthForm.fillAndValidate(InprogressCrown(dob = None))
