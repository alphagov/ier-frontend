...
class PreviouslyRegisteredStep @Inject() (val serialiser: JsonSerialiser,
  ...
  val editPostRoute = routes.PreviouslyRegisteredController.editPost
  val stepPostRoute = routes.PreviouslyRegisteredController.post

  def goToNext(currentState: InprogressOverseas): SimpleResult = {
    Redirect("/register-to-vote/broken")
  }
