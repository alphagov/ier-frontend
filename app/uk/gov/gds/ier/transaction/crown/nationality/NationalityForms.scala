...
trait NationalityConstraints extends FormKeys with ErrorMessages {
  ...
  lazy val atleastOneOtherCountryIfHasOtherCountry = Constraint[InprogressCrown] (
  ...
  lazy val notTooManyNationalities = Constraint[InprogressCrown](keys.nationality.key) {
    application =>
      val numberOtherCoutries = application.nationality.foldLeft(0) {
        (zero, nationality) => nationality.otherCountries.size
      }
  ...
  lazy val nationalityIsChosen = Constraint[InprogressCrown](keys.nationality.key) {
    application =>
  ...
  private def otherCountryIsValid(i:Int) = Constraint[InprogressCrown](
    keys.nationality.otherCountries.key
  ) { application =>
