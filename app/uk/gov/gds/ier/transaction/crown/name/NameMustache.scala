...
trait NameMustache extends StepMustache {
  ...
  case class NameModel(
    ...
    def transformFormStepToMustacheData(form: ErrorTransformForm[InprogressCrown],
                                        postUrl: String, 
  ...
  def nameMustache(form: ErrorTransformForm[InprogressCrown], call: Call, backUrl: Option[String]): Html = {
    val data = transformFormStepToMustacheData(form, call.url, backUrl)
    val content = Mustache.render("crown/name", data)
